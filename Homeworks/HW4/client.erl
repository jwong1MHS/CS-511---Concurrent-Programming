...
do_join(State, Ref, ChatName) ->
    case maps:is_key(ChatName, State#cl_st.con_ch) of
		...
		false ->
			...
			receive
			{RoomPID, Ref, connect, History} ->
				Updated = maps:put(ChatName, RoomPID, State#cl_st.con_ch),
				UpdatedState = State#cl_st{con_ch = Updated},
...
do_leave(State, Ref, ChatName) ->
    ...
    case maps:is_key(ChatName, State#cl_st.con_ch) of
		true ->
			...
			receive 
				{_PID, Ref, ack_leave} ->
				Removed = maps:remove(ChatName, State#cl_st.con_ch),
				NewState = State#cl_st{con_ch = Removed},
				{ok, NewState}
			end;
